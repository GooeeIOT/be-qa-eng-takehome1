...
In it's simpliest form, a function could take a URL to make a GET request to and a status code to assert for the response. If the expected and actual status code differ, then return false otherwise return true. If you want to get fancier, have it it; ideally you'd want to report how the test failed rather than a boolean pass/fail to cut down on mean-time-to-diagnose.
